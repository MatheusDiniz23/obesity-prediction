"""
Script de Treinamento e Avaliação de Modelos
-------------------------------------------
Este script realiza o treinamento de múltiplos modelos de classificação,
avalia a performance utilizando validação cruzada estratificada e salva
o melhor modelo para uso em produção.
...
def train_and_evaluate():
    ...
    # 4. Obter pré-processador (Sem peso para evitar Data Leakage)
    preprocessor = get_preprocessor(include_weight=False)
    ...
    print(f"{'Modelo':<25} | {'F1 (Mean)':<10} | {'Acc (Mean)':<10}")
    print("-" * 50)
    ...
    print("-" * 50)
    print(f"Melhor Modelo Selecionado: {best_model_name} (F1: {best_f1:.4f})")
    ...
    print(f"- Pipeline completa: {model_path}")
